...
public class InMemoryUserRepositoryTest {
    ...
    public void init() {
        this.inMemoryUserRepository = new InMemoryUserRepository(Arrays.asList(
                ...
                new User(null, "Karolina", "Nowak", Gender.FEMALE)
        ));
    }
    ...
    public void findById_ShouldReturnEmptyOptionalForNonExistingId() {
        // given
        Long id = 5L;
        User expectedUser = new User(2L, "Jan", "Kowalski", Gender.MALE);

        // when
        Optional<User> actual = inMemoryUserRepository.findById(id);

        // then
        Assert.assertFalse("Is not empty. User not found", actual.isPresent());
    }

    @Test
    public void fingById_ShouldReturnOptionalWithExpectedUserForGivenId() {
        // given
        Long id = 3L;
